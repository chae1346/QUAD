...
# 시작과 동시에 yolo 실행. target_position 도달 시 최종 데이터 처리.
...
def Hovering(local_pos_pub, rate):
    global pose, last_detected, data_lock
    hovering_pub = rospy.Publisher("/hovering_state", Bool, queue_size=10)  # 호버링 상태 토픽
    rospy.sleep(0.5)

    hovering_pub.publish(True)  # 호버링 시작
    for _ in range(100):  # 호버링 유지
        local_pos_pub.publish(pose)
        rate.sleep()
    hovering_pub.publish(False)  # 호버링 종료
    ...
    with data_lock:
        if last_detected:
            ...
            rospy.logwarn("No detection data received from YOLO.")
