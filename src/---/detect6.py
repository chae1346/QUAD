...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if view_img:
                cv2.namedWindow(str(p), cv2.WINDOW_NORMAL)  # Create a resizable window
                cv2.resizeWindow(str(p), 1280, 720)  # Set window size
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default=ROOT / '2', help='file/dir/URL/glob, 0 for webcam')
